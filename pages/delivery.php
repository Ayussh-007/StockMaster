...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["create_delivery"])) {
  ...
  // Status is not in the UI; default to Ready
  $status        = "Ready";

  if ($customer !== "" && $delivery_date !== "") {
    $stmt = $conn->prepare(
      "INSERT INTO delivery (customer, delivery_date, priority, status)
       VALUES (?, ?, ?, ?)"
    );
    $stmt->bind_param("ssss", $customer, $delivery_date, $priority, $status);
    $stmt->execute();
    $stmt->close();
  }
...
$deliveries = [];
$result = $conn->query("SELECT * FROM delivery ORDER BY id DESC");
...
<html lang="en">
  ...
  <body
    class="bg-gray-50 text-slate-800 h-screen flex overflow-hidden relative"
  >
    <!-- Auth Overlay (Blocker) -->
    <div
      id="auth-overlay"
      class="fixed inset-0 z-[60] bg-gray-900/60 backdrop-blur-md hidden flex items-center justify-center transition-opacity duration-300"
    >
      <div
        class="bg-white p-8 rounded-3xl shadow-2xl max-w-md w-full text-center mx-4 transform scale-100 transition-transform"
      >
        <div
          class="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner"
        >
        ...
        <div class="space-y-3">
          <a
            href="login.php"
            class="flex items-center justify-center w-full py-3.5 bg-indigo-600 text-white rounded-xl font-semibold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 transform hover:-translate-y-0.5"
            >Log In</a
          >
          <a
            href="signup.php"
            class="flex items-center justify-center w-full py-3.5 bg-white border-2 border-gray-100 text-gray-700 rounded-xl font-semibold hover:border-gray-300 hover:bg-gray-50 transition-all"
            >Create Account</a
          >
        </div>
      </div>
    ...
    >
      ...
      >
        ...
        <div class="p-6 overflow-y-auto">
          <form class="space-y-6" method="POST" action="delivery.php">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="col-span-2">
                ...
                <select
                  ...
                  required
                >
                  <option value="Deco Addict">Deco Addict</option>
                  <option value="Azure Interior">Azure Interior</option>
                  <option value="Gemini Furniture">Gemini Furniture</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2"
    ...
    >
      ...
      <nav class="flex-1 overflow-y-auto p-4 space-y-1">
        ...
        <a
          ...
          class="flex items-center w-full p-3 rounded-lg mb-1 text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors"
          ><i data-lucide="arrow-left-right" class="w-5 h-5 mr-3"></i> Move
          History</a
        >
        ...
        <a
          href="settings.html"
          class="flex items-center w-full p-3 rounded-lg mb-1 text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors"
          ><i data-lucide="settings" class="w-5 h-5 mr-3"></i> Settings</a
        >
      </nav>
        ...
        <div id="user-profile" class="hidden">
          ...
          >
            <a
              ...
              ><i data-lucide="user" class="w-4 h-4 mr-2"></i> My Profile</a
            >
                ...
                <tbody class="divide-y divide-gray-100">
                  <?php if (!empty($deliveries)): ?>
                    <?php foreach ($deliveries as $del): ?>
                      <?php
                        $id       = (int)$del["id"];
                        // Generate reference like WH/OUT/00054
                        $reference = "WH/OUT/" . str_pad($id, 5, "0", STR_PAD_LEFT);
                        $customer  = htmlspecialchars($del["customer"]);
                        $date      = $del["delivery_date"];
                        $dateFormatted = $date ? date("M j, Y", strtotime($date)) : "";
                        $status    = htmlspecialchars($del["status"]);
                        $priority  = htmlspecialchars($del["priority"]);

                        // Status pill color (Ready green, others default)
                  ...
                  <?php else: ?>
                    <tr>
                      <td colspan="6" class="p-4 text-center text-gray-500">
                        No delivery orders found. Click "Create Delivery" to add one.
                      </td>
                    </tr>
                  <?php endif; ?>
                </tbody>
